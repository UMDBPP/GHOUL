...
/*
 * Constructs DallasTemperature with strong pull-up turned on. Strong pull-up is
 * mandated in DS18B20 datasheet for parasitic power (2 wires) setup.
 * (https://datasheets.maximintegrated.com/en/ds/DS18B20.pdf, p. 7, section
 * 'Powering the DS18B20').
 */
DallasTemperature::DallasTemperature(OneWire *_oneWire, uint8_t _pullupPin)
...
uint8_t DallasTemperature::getDS18Count(void) { return ds18Count; }
...
// whether a conversion is complete FALSE: function requestTemperature() etc
// will wait a set time (worst case scenario) for a conversion to complete
...
void DallasTemperature::blockTillConversionComplete(uint8_t bitResolution) {
  ...
  if (checkForConversion && !parasite) {
    ...
    while (!isConversionComplete() &&
           (millis() - start < MAX_CONVERSION_TIMEOUT))
      yield();
  } else {
...
// If optional argument deviceAddress is omitted the command is send to all
// devices Returns true if no errors were encountered, false indicates failure
...
// Sends command to one device to recall values from EEPROM to scratchpad by
// index Returns true if no errors were encountered, false indicates failure
...
// Sends command to one or more devices to recall values from EEPROM to
// scratchpad If optional argument deviceAddress is omitted the command is send
// to all devices Returns true if no errors were encountered, false indicates
// failure
bool DallasTemperature::recallScratchPad(const uint8_t *deviceAddress) {
  ...
  // Specification: Strong pullup only needed when writing to EEPROM (and temp
  // conversion)
  unsigned long start = millis();
  while (_wire->read_bit() == 0) {
    // Datasheet doesn't specify typical/max duration, testing reveals typically
    // within 1ms
    if (millis() - start > 20)
      return false;
    yield();
...
bool DallasTemperature::getAutoSaveScratchPad() { return autoSaveScratchPad; }
...
bool DallasTemperature::isAllZeros(const uint8_t *const scratchPad,
                                   const size_t length) {
...
bool DallasTemperature::alarmSearch(uint8_t *newAddr) {
  ...
  for (i = 0; i < 64; i++) {
    ...
    // if something vanishes during the search it will come up.
    if (a && nota)
      return false;
